angular.module("valeant.controllers")
    ...
    .controller("CountriesController", function ($scope, $http, toastr, $uibModal, $appConfig) {
        $scope.gridOptions = {
            data: "data",
            enableSorting: false,
            ...
            columnDefs: [
                { name: "Страна", field: "Name" },
                { name: "Зарубежная?", field: "IsForeign", type: "boolean", cellTemplate: '<checkbox ng-model="row.entity.IsForeign"></checkbox>' },
                { name: "СНГ?", field: "IsCis", type: "boolean", cellTemplate: '<checkbox ng-model="row.entity.IsCis"></checkbox>' },
            ...
            appScopeProvider: {
                ...
                editRow: function (grid, row) {
                    var modalInstance = $uibModal.open({
                        templateUrl: "app/templates/settings/modals/editCountryModal.html",
        ...
        $scope.reload = function () {
            $http({
                ...
                $scope.data = response;
                if ($appConfig.showSuccessToastr)
                    toastr.success("Данные стран получены!");
            }).error(function (error) {
                toastr.error("Ошибка!");
                throw error;
            });
