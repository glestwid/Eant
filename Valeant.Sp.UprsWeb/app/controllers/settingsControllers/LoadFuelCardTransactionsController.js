angular.module("valeant.controllers")
    ...
    .controller("LoadFuelCardTransactionsController", function ($scope, $state, $stateParams, $http, toastr, $q, $timeout, $uibModal, $appConfig) {
        $scope.formData = {
            name: undefined,
            data: undefined
        };
        ...
        $scope.reload = function () {
            $http({
                method: "GET",
                url: "./loadFuelCardTransactions/getAll",
                cache: false
                ...
                reader.onloadend = function (evt) {
                    if (evt.target.readyState === FileReader.DONE) { // DONE == 2
                        ...
                        $scope.$apply(function () {
                            ...
                            $http({
                                method: "POST",
                                url: "./LoadFuelCardTransactions/loadData",
                                ...
                                if ($scope.data.Success) {
                                    if ($appConfig.showSuccessToastr)
                                        toastr.success("Справочник обновлен");
                                }
                                else {
                                    if ($appConfig.showSuccessToastr)
