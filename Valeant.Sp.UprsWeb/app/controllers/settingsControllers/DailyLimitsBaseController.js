angular.module("valeant.controllers")
    ...
    .controller("DailyLimitsBaseController", function ($scope, $http, toastr, $uibModal, $appConfig) {
        $scope.gridOptions = {
            data: "data",
            enableSorting: false,
            ...
            columnDefs: [
                { name: "Название ставки", field: "RateName" },
                { name: "Ставка", field: "Limit" },
            ...
            appScopeProvider: {
                ...
                editRow: function (grid, row) {
                    var modalInstance = $uibModal.open({
                        templateUrl: "app/templates/settings/modals/editDailyLimitBaseModal.html",
                        controller: "EditDailyLimitBaseController",
                        resolve: {
                            row: function () {
        ...
        $scope.reload = function () {
            $http({
                method: "GET",
                url: "./dailyLimitsBase/getAll",
                cache: false
            }).success(function (response) {
                $scope.data = response;
                if ($appConfig.showSuccessToastr)
                    toastr.success("Данные ставок суточных получены!");
            }).error(function (error) {
                toastr.error("Ошибка!");
                throw error;
            });
